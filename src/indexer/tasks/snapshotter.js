...
async function snapshotPools(poolAddresses) {
    ...
    try {
        ...
        // Pass 2 logic (Simplified for Immediate mode)
        for (const p of pools) {
            ...
            if (task.type === 'pump') {
                ...
                if (accA && accB) {
                    // Logic from cycle... reuse or copy carefully
                    ...
                    if (nativePrice > 0) {
                        const finalPrice = nativePrice * solPriceCache; // Assuming quote is SOL/USDC-peg
                        await db.run(`UPDATE pools SET price_usd = $1, liquidity_usd = $2 WHERE address = $3`, [finalPrice, liquidityUsd, p.pool_address]);
                    }
                }
            }
...
async function runSnapshotCycle() {
    ...
    const db = getDB();
    const redis = getClient(); // USE REDIS FOR RESERVE CACHE
    const connection = getConnection();
    await updateSolPrice(db);
    ...
    try {
        while (keepFetching) {
            ...
            // --- SCALABILITY FIX: BATCH UPDATES IN TRANSACTION ---
            // Instead of firing 200 DB queries, we open ONE transaction client
            ...
            try {
                    ...
                    try {
                        if (task.type === 'pump') {
                            ...
                            if (data) {
                                ...
                                if (vaultA && vaultB) {
                                    await client.query(`UPDATE pools SET reserve_a = $1, reserve_b = $2 WHERE address = $3`, [vaultA.toString(), vaultB.toString(), p.pool_address]);
                                }
                            }
                        ...
                        if (nativePrice > 0) {
                            ...
                            // --- PERSISTENT VOLUME LOGIC (REDIS) ---
                            let approxVolume = 0;
                            const volKey = `vol:${p.pool_address}`;
                            
                            if (redis) {
                                const prevReserve = await redis.get(volKey);
                                if (prevReserve) {
                                    const delta = Math.abs(currentQuoteReserve - parseFloat(prevReserve));
                                    if (delta > 0.000001 && delta < (currentQuoteReserve * 0.5)) { 
                                        // Ignore massive jumps (likely reorg/liquidity add)
                                        approxVolume = delta;
                                    }
                                }
                                // Set new reserve, 5 mins TTL is enough to bridge polling gaps
                            ...
                            await client.query(
                                `INSERT INTO candles_1m (pool_address, timestamp, open, high, low, close, volume) 
                                 ...
                                 DO UPDATE SET close = $3, volume = candles_1m.volume + $4, high = GREATEST(candles_1m.high, $3), low = LEAST(candles_1m.low, $3)`, 
                                [p.pool_address, timestamp, finalPriceUsd, approxVolume]
                            );
                            
                            await client.query(
                                `UPDATE pools SET price_usd = $1, liquidity_usd = $2 WHERE address = $3`, 
                                [finalPriceUsd, liquidityUsd, p.pool_address]
                            );
                        }
                    } catch (e) {
                        // logger.warn(`Skipping pool ${p.pool_address}: ${e.message}`);
                    }
                }
                ...
                logger.error('Snapshot TX Failed', txError);
            } finally {
                client.release();
            }
