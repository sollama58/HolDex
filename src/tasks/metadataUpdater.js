...
async function updateMetadata(deps) {
    ...
    try {
        ...
        logger.info(`Running Metadata Update for ${tokens.length} tokens...`);
        ...
        for (const t of tokens) {
            try {
                ...
                // 4. Calculate Price Changes (STRICTLY PRICE BASED)
                ...
                if (currentPrice > 0) {
                    ...
                    const getPriceAt = async (targetTime) => {
                        ...
                        // Fallback: If no history, use oldest known candle (Change since Inception)
                        if (!res) {
                            ...
                            `, [bestPool.address]);
                            // If we have a candle, use its Open. If we have NO candles, assume started at CurrentPrice (0% change)
                            return oldest ? (oldest.open || oldest.close) : currentPrice;
                        }
                        return res.close;
                    };
                ...
                logger.warn(`Meta Update Failed (${t.mint}): ${err.message}`);
            }
        }
    } catch (e) {
