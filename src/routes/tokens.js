...
function init(deps) {
    ...
    async function verifyPayment(signature, payerPubkey) {
        ...
        let tx = null;
        try { tx = await solanaConnection.getParsedTransaction(signature, { commitment: 'confirmed', maxSupportedTransactionVersion: 0 }); } catch (err) {}
        return true; 
    }
    ...
    async function processDexPairs(mint, pairs) {
        ...
        // 2. Prepare Base Data from the "Best" Pair ( DexScreener sorts by liquidity usually)
        // We use this for basic metadata (name, image)
        ...
        const baseData = {
            ...
            volume24h: 0, // Will be recalculated by aggregator
        ...
        // 3. Aggregate Volume & Price from the DB (using the pools we just inserted)
    ...
    router.get('/token/:mint/candles', async (req, res) => {
        ...
        try {
            const result = await smartCache(cacheKey, 60, async () => {
                ...
                // This ensures the chart reflects the "real" price action
    ...
    router.get('/tokens', async (req, res) => {
        ...
        try {
            const result = await smartCache(cacheKey, 5, async () => {
                ...
                let shouldFetchExternal = searchTerm.length > 0 && searchTerm.length >= 3;
                ...
                if (shouldFetchExternal) {
                    ...
                    const externalTokens = await smartCache(extCacheKey, 300, async () => { 
                        try {
                            ...
                            if (dexRes.data?.pairs) {
                                ...
                                // Group by Mint to avoid duplicates in result
                                const uniqueTokens = {};
                                for (const pair of validPairs) {
                                    const mint = pair.baseToken.address;
                                    if (!uniqueTokens[mint]) {
                                        uniqueTokens[mint] = {
                                            mint, 
                                            ...
                                            isExternal: true,
                                            rawPairs: [] // collect pairs for this mint
                                        };
                                    }
                                    uniqueTokens[mint].rawPairs.push(pair);
                                }
                    ...
                    if (externalTokens && externalTokens.length > 0) {
                        // If exact Address Search, save and index everything immediately
                        if (isAddressSearch) {
    ...
    router.get('/token/:mint', async (req, res) => {
        ...
        const result = await smartCache(cacheKey, 30, async () => {
            ...
            try {
                ...
                if (dexRes.data?.pairs) {
                    pairs = dexRes.data.pairs; // don't sort here, processDexPairs handles logic
                    
                    if (pairs.length > 0) {
                        // AUTO-INDEX: Update aggregates whenever individual token page is loaded
                        // This ensures "stale" volume data gets refreshed if user visits page
                        const updatedData = await processDexPairs(mint, pairs);
                        
                        // Merge updated data for response
                        if (updatedData) {
    ...
    router.post('/admin/approve-update', requireAdmin, async (req, res) => {
        ...
        // Trigger re-index of pairs to be safe
        try {
